...
namespace Word_Analyzer
{
	...
	{
		public List<List<(char letter, int sum)>> letterPos;
		public List<(char letter, int sum)> letterInc;
	...
	public class Analyzer : IDisposable
	{
		...
		public Analyzer(byte length, string connection, string table, string column) : this(length, connection, table, column, ConfigurationManager.ConnectionStrings["tmpTablesRootName"]?.ConnectionString ?? "edit") { }
		...
		private void ResetWorkingTables()
		{
			cmd.CommandText = GetCreateWordTableCommand();
			cmd.ExecuteNonQuery();
		}

		private List<string> FetchCandidates(string tableName)
		{
			List<string> candidates = new List<string>();
			cmd.CommandText = "SELECT " + columnString + " FROM " + tableName + ";";
		...
		private string GetRemReqPosCommand(byte charPos, char letter, string table)
		{
			string command = 
				"\nDELETE FROM " + table + 
		...
		private string GetRemReqNPosCommand(char letter, string table)
		{
			string command =
				"\nDELETE FROM " + table + 
		...
		private string GetRemBannedCommand(char letter, string table)
		{
		...
		private string GetRemInvalPosCommand(byte charPos, char letter, string table)
		{
			string command =
				"\nDELETE FROM " + table + 
		...
		/// </summary>
		/// <returns></returns>
		public int RemReqPos(string table = null)
		{
			string targetTable = table ?? tableString;
			int sum = 0;
			foreach ((char letter, byte pos) in reqPos)
			{
				cmd.CommandText = GetRemReqPosCommand(pos, letter, targetTable);
		...
		/// </summary>
		public int RemReqNPos(string table = null)
		{
			string targetTable = table ?? tableString;
			int sum = 0;
			foreach (char letter in reqLetters)
			{
				cmd.CommandText = "DELETE FROM " + targetTable + " WHERE " + columnString + " NOT LIKE '%" + letter + "%';";
				sum += cmd.ExecuteNonQuery();
			}
			return sum;
		}
		...
		/// </summary>
		/// <returns></returns>
		public int RemBanned(string table = null)
		{
			string targetTable = table ?? tableString;
			int sum = 0;
			foreach (char letter in bannedLetters)
			{
				...
				cmd.CommandText = "DELETE FROM " + targetTable + " WHERE " + columnString + " NOT LIKE '" + regex + "';";
				sum += cmd.ExecuteNonQuery();
			}
			return sum;
		}
		...
		/// </summary>
		/// <returns></returns>
		public int RemInvalPos(string table = null)
		{
			string targetTable = table ?? tableString;
			int sum = 0;
			foreach ((char letter, byte pos) in bannedPos)
			{
				cmd.CommandText = "DELETE FROM " + targetTable + " WHERE " + columnString + " LIKE " + CreateRegex(pos, letter) + ";";
				sum += cmd.ExecuteNonQuery();
			}
			return sum;
		}
		...
		public List<List<(char, int)>> ComputePosAndSort(List<char> alphabet, string table)
		{
			List<List<(char, int)>> results = ComputePos(alphabet, table);
		...
		{
			return ComputePosAndSort(alphabet, tableString);
		}
		...
		/// <summary>
		/// Returns full analysis including candidate list, and uses a per-run working table so the base word list stays untouched.
		/// 0 - char not in word | 1 - char in word, not in position | 2 - char in word, in correct position
		/// </summary>
		public AnalysisResult AnalyzeWithCandidates(List<(char, byte)> feedback)
		{
			StartConnection();
			...
			try
			{
				...
				//Remove invalid words from dataset using the working table
				RemReqPos(stateWordTable);
				RemReqNPos(stateWordTable);
				RemBanned(stateWordTable);
				RemInvalPos(stateWordTable);

				//Compute stats off the filtered working set
				List<List<(char, int)>> letterPos = ComputePosAndSort(alphabet, stateWordTable);
				List<(char, int)> letterInc = ComputeInc(alphabet, stateWordTable);
				List<string> candidates = FetchCandidates(stateWordTable);
		...
		/// <summary>
		/// Backwards-compatible tuple-returning API for existing callers.
		/// </summary>
		public (List<List<(char, int)>>, List<(char, int)>) Run(List<(char, byte)> feedback)
		{
			AnalysisResult result = AnalyzeWithCandidates(feedback);
			return (result.letterPos, result.letterInc);
		}
